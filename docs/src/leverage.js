...
// MEXC sends no CORS headers, so a browser can't call it. The scheduled scanner
// can, and publishes what it found — the page hydrates from that instead.
export function hydrate(map) {
  const next = new Map(Object.entries(map || {}).map(([k, v]) => [k, Number(v)]).filter(([, v]) => v > 0));
...
export const dump = () => Object.fromEntries(cache);
